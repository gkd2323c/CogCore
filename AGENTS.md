...
Cognitive kernel ("long-term cognitive runtime") for LLM agents. Engineering rewrite of the AP paper (2026). **M3.6 complete** — 32 modules, 31 test files, 390 tests.
...
- **`src/cogcore/`** — 32 Python modules (`types.py`, `pipeline.py`, `hdb.py`, `graph.py`, `llm_bridge.py`, `service.py`, `agent.py`, `tools*.py`, `self_iteration.py`, `main.py`, `run.py`, …)
- **`app/`** — FastAPI app (endpoints: chat, diary, status, ws). Depends on `src/cogcore/`.
- **`tests/`** — 31 test files. `test_api.py` uses `cogcore_data_api_test/` as temp data dir (auto cleaned).
- **`scripts/`** — 15+ demo/verification scripts. `verify_all.py` is the main one.
- **`experiments/E01/`…`E20/`** — one dir per experiment. See `docs/CogCore-验证矩阵.md`.
